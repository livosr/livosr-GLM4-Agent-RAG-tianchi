import json
import sys
sys.path.append(r'/Users/bytedance/Downloads/autoheader-master/cache/cache_distinct/yuyuelin_b')
...
if __name__ == '__main__':
    sub_question_filename = "test.json"
    ...
    for data in lines[2:]:
        ...
        try:
            ...
            # 去掉外层的引号再进行第二次反序列化
            sub_question_str = sub_question_str.strip('"')
            sub_question_str = sub_question_str.replace('\\"', '"')

            sub_question_dict = json.loads(sub_question_str)

            for key, value in sub_question_dict.items():
                sub_list.append({"sub_question": key, "func": value.split(" ")[0]})
